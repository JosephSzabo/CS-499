    ...
    public List<InventoryItem> getAllItems() {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                ...
                // Log the column indices for debugging
                Log.d("Database", "Column indices - Name: " + nameIndex + ", Quantity: " + quantityIndex + ", Unit: " + unitIndex);

                // Ensure valid column indices
                if (nameIndex != -1 && quantityIndex != -1 && unitIndex != -1) {
                    // Safe to retrieve data from the cursor
                    String name = cursor.getString(nameIndex);
                    int quantity = cursor.getInt(quantityIndex);
                    String unit = cursor.getString(unitIndex);
                } else {
                    // Log error if any column is missing
                    Log.e("DatabaseError", "Column indices are invalid. Check if columns exist in the database.");
                }
